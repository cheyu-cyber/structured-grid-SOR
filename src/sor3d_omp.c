/*****************************************************************************
  ...
  3D extension of sor2d_omp.c.  Same Lab-6 OMP scaffolding
  (detect_threads_setting, arr3 ADT, double data_t) plus the temporal
  super-step from sor3d_cpu.c.
...
/* ============================ Baseline 3D OMP ============================== */
void sor3d_baseline_omp(data_t *src, data_t *dst, int N, int iters, double omega)
{
  for (int it = 0; it < iters; it++) {
    ...
    if (fabs(dst[IDX3(N-2,N-2,N-2,N,N)]) > 10.0 * (MAXVAL - MINVAL)) {
      fprintf(stderr, "sor3d_baseline_omp: SUSPECT DIVERGENCE iter = %d\n", it);
      break;
    }
