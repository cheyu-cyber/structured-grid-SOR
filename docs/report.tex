...
measure its unimodal optimum in §\ref{sec:cpu-results}.
...
\paragraph{Size sweep.} Figure~\ref{fig:size-sweep} shows the
...
\caption{Pthreads decomposition $\times$ schedule.}
\end{subfigure}
...
\paragraph{Pthread spawn-overhead.} Figure~\ref{fig:spawn} is
...
\paragraph{Tuning sweep.} Figure~\ref{fig:gpu-tile-halo} is the
...
\paragraph{Cross-tier headline.} At $N{=}2050$, iters=96, 8
threads:
...
\bottomrule
\end{tabular}
\caption{Cross-tier CPE summary.}
\label{tab:headline}
\end{table}
